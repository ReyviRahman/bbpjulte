...
@section('content')
    ...
    <div class="p-6 text-gray-900 dark:text-gray-100">
      ...
      <div class="flex sm:flex-row flex-col items-center justify-between mb-6">
        <div class="flex items-center gap-1">
          <a href="{{ route('admin.form-permohonan.daftar-input') }}"
          ...
          <h2 class="text-lg font-semibold">Daftar Permohonan {{ $kategori }} </h2>
        </div>
          ...
          <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-lg bg-white dark:bg-gray-800"
            ...
            <div class="text-left">
              <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-200 mb-2">
                Tambah Data {{ ucfirst($kategori ?? 'Baru') }}
              </h3>

              <form action="{{ route('admin.form-permohonan.store') }}" method="POST" class="mt-4"
                enctype="multipart/form-data">
                @csrf

                {{-- Input Hidden Category (PENTING) --}}
                ...
                @if(isset($kategori) && ($kategori == 'Unggah surat permohonan' || $kategori == 'Unggah lampiran permohonan'))
                  <div class="mb-4">
                    <label for="fileTemplateInput" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                      Upload File Template (Opsional)
                    </label>

                    <input type="file" id="fileTemplateInput" name="file_template" class="mt-1 block w-full text-sm text-gray-500
                                       border border-gray-300 rounded-md cursor-pointer bg-gray-50 focus:outline-none
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Format: PDF/Docx.</p>
                  </div>
                @endif
                  ...
                  <div class="text-sm font-medium text-gray-900 dark:text-gray-100">
                    ...
                    @if(!empty($item->file_template))
                      @php
                        ...
                        // Cek apakah katanya lebih dari 1
                        if ($count > 1) {
                          // Ambil 2 kata terakhir gabung jadi string
                          $linkText = implode(' ', array_slice($parts, -2));

                          // Ambil sisa kata di depan (semua kecuali 2 terakhir)
                          $frontText = implode(' ', array_slice($parts, 0, $count - 2));
                        } else {
                          // Jika cuma ada 1 kata (misal: "SK"), ya semuanya jadi link
                          $linkText = $item->name;
                          $frontText = '';
                        }
                      @endphp

                      {{-- Tampilkan teks depan (Hanya jika ada isinya) --}}
                      @if($frontText)
                        {{ $frontText }}
                      @endif

                      {{-- Tampilkan 2 kata terakhir sebagai Link --}}
                      <a href="{{ Storage::url($item->file_template) }}" target="_blank"
                        class="text-blue-600 font-bold hover:underline hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300"
                        title="Klik untuk mengunduh template">
                        {{$linkText }}
                      </a>

                    @else
                      {{-- Jika tidak ada file, tampilkan teks biasa full --}}
                      {{ $item->name }}
                    @endif
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                  <div class="flex items-center justify-center space-x-4">
                    @if($item->category == 'Layanan')
                      <a href="{{ route('admin.form-permohonan.subs.index', ['form_id' => $item->id]) }}"
                        class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 action-btn"
                        title="Edit"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                    ...
                    <button type="button"
                      onclick="openEditModal('{{ $item->id }}', '{{ $item->name }}', '{{ route('admin.form-permohonan.update', $item->id) }}', '{{ $item->category }}')"
                      class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 action-btn"
                      title="Edit">
                    ...
                    {{-- TOMBOL HAPUS --}}
                    <form action="{{ route('admin.form-permohonan.destroy', $item->id) }}" method="POST"
              ...
              <tr>
                <td colspan="3" class="px-6 py-10 text-center text-gray-500">
                  <div class="flex flex-col items-center">
                    ...
                    <p class="font-semibold">Belum Ada Data {{ ucfirst($kategori) }}</p>
  ...
  <div id="editModal" onclick="if(event.target === this) closeEditModal()"
    ...
    <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-lg bg-white dark:bg-gray-800"
      ...
      <div class="text-left">
        <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-200 mb-2">
          Edit Data {{ ucfirst($kategori ?? '') }}
        </h3>

        {{-- Form Edit (Action akan diisi via JS) --}}
        <form id="editForm" method="POST" class="mt-4" enctype="multipart/form-data">
          @csrf
          ...
          <div id="editFileContainer" class="mb-4 hidden">
            <label for="editFileInput" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
              Ganti File Template (Opsional)
            </label>
            <input type="file" id="editFileInput" name="file_template"
              class="mt-1 block w-full text-sm text-gray-500 border border-gray-300 rounded-md cursor-pointer bg-gray-50
                                file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold
                                file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300">
...
@push('scripts')
  <script>
    function openEditModal(id, name, updateUrl, category) {
      // 1. Isi value input nama
      document.getElementById('editNameInput').value = name;

      // 2. Ubah action form
      document.getElementById('editForm').action = updateUrl;

      // 3. Reset input file (biar kosong setiap buka modal)
      document.getElementById('editFileInput').value = '';

      // 4. Cek Kategori untuk menampilkan/menyembunyikan input file
      const fileContainer = document.getElementById('editFileContainer');

      // Cek jika kategori adalah 'Unggah surat permohonan' ATAU 'Unggah lampiran permohonan'
      if (category == 'Unggah surat permohonan' || category == 'Unggah lampiran permohonan') {
        fileContainer.classList.remove('hidden'); // Munculkan
      } else {
        fileContainer.classList.add('hidden');    // Sembunyikan
      }
