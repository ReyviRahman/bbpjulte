    ...
    <div class="p-6 text-gray-900 dark:text-gray-100">
      ...
      <div class="flex sm:flex-row flex-col items-center justify-between mb-6">
        <div class="flex items-center gap-1">
          <a href="{{ route('admin.form-permohonan.index', ['category' => $parent->category]) }}"
          ...
          <h2 class="text-lg font-semibold">Daftar Permohonan Sub Kategori {{ $parent->name }} </h2>
        </div>
        ...
        <div id="inputModal" onclick="if(event.target === this) this.style.display='none'"
          ...
          <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-lg bg-white dark:bg-gray-800"
            ...
            <div class="text-left">
              ...
              <form action="{{ route('admin.form-permohonan.subs.store') }}" method="POST" class="mt-4">
                @csrf
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                  <div class="flex items-center justify-center space-x-4">
                    @if($item->category === 'layanan')
                      <a href="{{ route('admin.form-permohonan.subs.index', $item->id) }}"
                    ...
                    <button type="button"
                      onclick="openEditModal('{{ $item->id }}', '{{ $item->name }}', '{{ route('admin.form-permohonan.subs.update', $item->id) }}')"
                      class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 action-btn"
                      title="Edit">
                    ...
                    {{-- TOMBOL HAPUS --}}
                    <form action="{{ route('admin.form-permohonan.subs.destroy', $item->id) }}" method="POST"
