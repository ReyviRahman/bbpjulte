...
@php
  ...
  $listLowerCase = array_map('strtolower', $kategoriBerbobot);
  $isScoredCategory = in_array($currentCategory, $listLowerCase);
@endphp
    ...
    <div class="p-6 text-gray-900 dark:text-gray-100">
      ...
      <div class="flex sm:flex-row flex-col items-center justify-between mb-6">
        <div class="flex items-center gap-1">
          <a href="{{ route('admin.form-skm.daftar-input') }}"
          ...
          <h2 class="text-lg font-semibold">Daftar SKM {{ $kategori }} </h2>
        </div>
        ...
        <div id="inputModal" onclick="if(event.target === this) this.style.display='none'"
          ...
          <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-lg bg-white dark:bg-gray-800"
            ...
            <div class="text-left">
              ...
              <form action="{{ route('admin.form-skm.store') }}" method="POST" class="mt-4">
                @csrf
                ...
                @if($isScoredCategory)
                  <div class="mb-4">
                    <label for="scoreInput" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                      Nilai / Poin (Bobot) <span class="text-red-500">*</span>
                    </label>
        ...
        <table
          ...
          <thead class="bg-gray-50 dark:bg-gray-700/50">
            <tr>
              ...
              {{-- Kolom 3: Score (HANYA MUNCUL JIKA KATEGORI BERBOBOT) --}}
              @if($isScoredCategory)
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            ...
            @forelse ($data as $index => $item)
              <tr class="table-hover-row">
                ...
                {{-- 3. Nilai Score (HANYA MUNCUL JIKA KATEGORI BERBOBOT) --}}
                @if($isScoredCategory)
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                  <div class="flex items-center justify-center space-x-4">
                    ...
                    <button type="button" onclick="openEditModal(
                        '{{ $item->id }}', 
                        '{{ $item->name }}', 
                        '{{ $item->score ?? '' }}',                   {{-- Kirim Score lama --}}
                        '{{ route('admin.form-skm.update', $item->id) }}',
                        {{ $isScoredCategory ? 'true' : 'false' }}    {{-- Kirim status TRUE/FALSE --}}
                    )" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 action-btn"
                      title="Edit">
                    ...
                    {{-- Tombol Hapus --}}
                    <form action="{{ route('admin.form-skm.destroy', $item->id) }}" method="POST"
            ...
            @empty
              {{-- Tampilan Kosong --}}
              <tr>
                <td colspan="{{ $isScoredCategory ? 4 : 3 }}" class="px-6 py-10 text-center text-gray-500">
    ...
    <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-lg bg-white dark:bg-gray-800"
      ...
      <div class="text-left">
        ...
        <form id="editForm" method="POST" class="mt-4">
          ...
          {{-- BARU: Input Score (Default Hidden) --}}
          <div id="editScoreContainer" class="mb-4 hidden">
            <label for="editScoreInput" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
              Nilai / Poin (Bobot) <span class="text-red-500">*</span>
            </label>
            <input type="number" id="editScoreInput" name="score"
              class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
...
@push('scripts')
  <script>
    // Tambahkan parameter score dan hasScore
    function openEditModal(id, name, score, updateUrl, hasScore) {
      // 1. Isi value nama
      ...
      // 3. Logika Tampilkan Input Score
      const scoreContainer = document.getElementById('editScoreContainer');
      const scoreInput = document.getElementById('editScoreInput');

      if (hasScore) {
        // Jika TRUE (Kategori Berbobot), tampilkan & isi nilai
        scoreContainer.classList.remove('hidden');
        scoreInput.value = score;
        scoreInput.setAttribute('required', 'required'); // Wajib isi jika muncul
      } else {
        // Jika FALSE, sembunyikan & kosongkan
